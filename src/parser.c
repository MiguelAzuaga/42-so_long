...
/*   Updated: 2025/07/24 10:20:19 by mqueiros         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "so_long.h"

static void	set_component(int component[3], int y, int x)
{
	component[0]++;
	component[1] = y;
	component[2] = x;
...
static int	find_components(t_map *map)
{
	...
	while (map->arr[i])
	{
		...
		{
			if (map->arr[i][j] == 'P')
				set_component(map->p, i, j);
			else if (map->arr[i][j] == 'E')
				set_component(map->e, i, j);
			else if (map->arr[i][j] == 'C')
			{
				map->c_count += 1;
				set_component(map->c, i, j);
			}
		}
		i++;
	}
	if (map->p[0] != 1 || map->e[0] != 1 || map->c[0] == 0)
		return (3);
	return (0);
}
